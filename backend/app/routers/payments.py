...
def create_nowpayments_invoice(amount: float, currency: str, merchant_id: int):  # type: ignore[no-untyped-def]
    ...
    if not settings.NOWPAYMENTS_API_KEY:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="NOWPayments API key not configured"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail=f"Failed to create NOWPayments invoice: {str(e)}"
        )
...
def create_payment(
    request: CreatePaymentRequest, merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)
    ...
    if currency_upper == "XRP":
        try:
            ...
            db.commit()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create XRP payment: {str(e)}"
            )
...
def get_payment(
    payment_id: int, merchant: Merchant = Depends(get_current_merchant), db: Session = Depends(get_db)
):  # type: ignore[no-untyped-def]
    """Get payment details"""
